...
/** Updates task thumbnail cache size and preloads if necessary. */
class UpdateThumbnailCacheSizeUseCase
@Inject
constructor(
    ...
    fun updateCacheSize(shouldPreloadIfNeeded: Boolean) {
        if (!enableLowResThumbnailPreloading()) return

        if (taskThumbnailDataSource.updateCacheSizeAndRemoveExcess() && shouldPreloadIfNeeded) {

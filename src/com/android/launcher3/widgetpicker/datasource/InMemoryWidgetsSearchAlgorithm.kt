...
class InMemoryWidgetSearchAlgorithm
@Inject
constructor(@BackgroundContext private val backgroundContext: CoroutineContext) :
    WidgetsSearchAlgorithm {
    override suspend fun initialize() {}

    override suspend fun searchWidgets(query: String, corpus: List<WidgetApp>): List<WidgetApp> =
        coroutineScope {
            // Ideally, uses search only one word, but supporting multiple words.
            val queryWords =
                query.trim().lowercase().split("\\s+".toRegex()).filter { it.isNotBlank() }
            ...
            val results =
                corpus.map { widgetApp ->
                    async(backgroundContext) { matchAndScoreWidgetApp(widgetApp, queryWords) }
                }
    ...
    private fun matchAndScoreWidgetApp(
        ...
        return if (appTitleScore > 0) {
            ...
            widgetApp.copy(widgets = scoredWidgets.map { it.first }) to totalAppScore
        }
    }
    ...
    private fun matchAndCalculateScore(
        ...
        var totalScore = 0
        val wordsInTarget =
            targetText.lowercase().split("\\s+".toRegex()).filter { it.isNotBlank() }
        ...
        for (queryWord in queryWords) {
            ...
            for (targetWord in wordsInTarget) {
                wordScore =
                    wordScore.coerceAtLeast(
                        calculateSingleWordScore(queryWord, targetWord, matchType)
                    )
            }

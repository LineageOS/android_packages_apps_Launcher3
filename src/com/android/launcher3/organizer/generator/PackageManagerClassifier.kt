...
/**
 * A [Classifier] that uses Android's package manager (via [LauncherApps]) to classify apps based on
 * their system-defined categories.
 *
 * It maps system constants (e.g., [ApplicationInfo.CATEGORY_GAME]) to user-friendly topic names.
 */
class PackageManagerClassifier(private val context: Context) : Classifier {
     ...
     */
    override suspend fun classify(items: List<ItemInfo>): List<TopicClassifiedItem> {
        return items.mapNotNull { item ->
            val intent = item.intent
            val appInfo =
                if (intent != null) {
                    launcherApps?.resolveActivity(intent, item.user)?.applicationInfo
                } else {
                    null
                }

            val categoryTopic = appInfo?.let { getCategoryTopic(it.category) }
    ...
    private fun getCategoryTopic(category: Int): String? {

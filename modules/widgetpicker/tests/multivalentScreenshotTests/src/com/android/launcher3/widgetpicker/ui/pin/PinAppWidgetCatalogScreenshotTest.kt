...
import com.android.launcher3.widgetpicker.ui.testdata.ScreenshotTestData.Companion.createBitmapPreview
...
class PinAppWidgetCatalogScreenshotTest(emulationSpec: DeviceEmulationSpec) {
    ...
    fun pinAppCatalog() =
        testScope.runTest {
            val testApp = testData.widgetApps().first { it.widgets.size == 1 }
            val testAppId = testApp.id
            val testWidget = testApp.widgets[0]
            val widgetPickerComponent = createWidgetPickerComponent()
            screenshotRule.screenshotTest(
                goldenIdentifier = "pinAppCatalog",
                beforeScreenshot = {
                    advanceUntilIdle()
                    runCurrent()
                },
